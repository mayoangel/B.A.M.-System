...
class ParentRepository:
    ...
    # Buscar padres/tutores por correo (email es UNIQUE); usado para reutilizar
    # al mismo tutor cuando se registra a un segundo hijo/a.
    def get_parent_by_email(self, email: str) -> Parents:
    ...
    # Actualizar tutor por su llave primaria (pantalla de administración)
    def update_parent_by_id(self, parent_id: int, new_data: dict) -> bool:
        db_parent = self.get_parent_by_id(parent_id)
        if db_parent:
            for key, value in new_data.items():
                setattr(db_parent, key, value)
            self.db.commit()
